...
class FlaxInfusingStableDiffusionPipeline(FlaxDiffusionPipeline):
    ...
    def _generate(
        ...
        biasing_latents = [b_latents * self.scheduler.init_noise_sigma for b_latents in biasing_latents] #TODO Verify the need
        ...
        def loop_body(step, args):
            ...
            noise_pred = self.unet.apply(
                ...
                latent_model_biases=latent_model_biases,
    ...
    def __call__(
        ...
        params: Union[Dict, FrozenDict],
        prng_seed: jax.random.PRNGKey,
        biasing_images: int, #List[PIL.Image.Image], # Union[torch.FloatTensor, PIL.Image.Image],
        layer_biases = int, #[.1,.1,.1,.1],
        num_inference_steps: int = 50,
        height: int = 512,
        width: int = 512,
        guidance_scale: float = 7.5,
        ...
        **kwargs,
        ...
        if jit:
            images = _p_generate(
                self, prompt_ids, params, prng_seed, None, None, num_inference_steps, height, width, guidance_scale, latents, debug
            )
        else:
...
def _p_generate(
    pipe, prompt_ids, params, prng_seed, biasing_images, layer_biases, num_inference_steps, height, width, guidance_scale, latents, debug
):
