...
#from .flax_unet_base import FlaxInfusionUNetModel
from .flax_infusion_unet_full import FlaxInfusionUNetModel
...
class FlaxInfusingStableDiffusionPipeline(FlaxDiffusionPipeline):
    ...
    def _generate(
        ...
        bias_decay: float = 0.994,
    ):

        biasing_images = jnp.squeeze(biasing_images)
        layer_biases = jnp.squeeze(layer_biases)
        ...
        if latents is None:
            ...
            latents = jax.random.normal(prng_seed, shape=latents_shape, dtype=jnp.float32) #*.95
        else:
        ...
        def loop_body(step, args):
            ...
            layer_biases = [bias*bias_decay for bias in layer_biases]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if jit:
            biasing_images = np.asarray(biasing_images)
            biasing_images = np.array([biasing_images] * jax.device_count())
            biasing_images = shard(biasing_images)
            layer_biases = np.array(layer_biases * jax.device_count())
            layer_biases = shard(layer_biases)

            images = _p_generate(
                self, prompt_ids, params, prng_seed, num_inference_steps, height, width, guidance_scale, latents, debug, biasing_images, layer_biases, bias_decay
            )
        else:
            images = self._generate(
                prompt_ids, params, prng_seed, num_inference_steps, height, width, guidance_scale, latents, debug, biasing_images, layer_biases, bias_decay
            )
...
@partial(jax.pmap, static_broadcasted_argnums=(0, 4, 5, 6, 7, 9, 12))
def _p_generate(
    pipe, prompt_ids, params, prng_seed, num_inference_steps, height, width, guidance_scale, latents, debug, b_latents, layer_biases, bias_decay
):
    return pipe._generate(
        prompt_ids, params, prng_seed, num_inference_steps, height, width, guidance_scale, latents, debug, b_latents, layer_biases, bias_decay
    )
