...
class UNet2DBiasedConditionModel(ModelMixin, ConfigMixin):
    r"""
    ...
    def forward(
        self,
        sample: torch.FloatTensor,
        biasSamples: List[torch.FloatTensor],
        ...
        if self.config.center_input_sample:
            sample = 2 * sample - 1.0
            biasSamples= [2 * b_sample- 1.0 for b_sample in biasSamples]
        ...
        for downsample_block in self.down_blocks:
            ...
            if hasattr(downsample_block, "attentions") and downsample_block.attentions is not None:
                ...
                #Same thing for bias samples
                for b_sample in biasSamples:
                  biasSample, res_biasSample = downsample_block(
                      hidden_states=b_sample,
                      temb=emb,
                      encoder_hidden_states=encoder_hidden_states,
                  )
                ...
                for b_sample in biasSamples:
                  biasSample, res_biasSample = downsample_block(hidden_states=b_sample, temb=emb)
                  new_biasSamples.append(biasSample)
                  new_res_biasSamples.append(res_biasSample)
            ...
            BIAS_1_LAYER = False
            BIAS_ALL_LAYERS = True
            if(BIAS_1_LAYER):
              if(block_num == layer_to_add):
                  sample = sample*1/2 + sample2*1/2
                  #sample = sample * 1/5 + sample2*4/5
                  res_samples = tuple([tup[0]*1/2 + tup[1]*1/2 for tup in zip(res_samples, res_samples2)])
            ...
            if(BIAS_ALL_LAYERS):
              ...
              sum_res_biasSamples = [sum(tup)/len(tup) for tup in zip(*res_biasSamples)] #TODO TRY MEAN
        ...
        #sample = sample*1/2 + sample2*1/2
        #res_samples = tuple([sum(tup)/2 for tup in zip(res_samples, res_samples2)])
